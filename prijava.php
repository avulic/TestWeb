<?php
    ...
    function prijava() {
        ...
        $korIme = $sql -> real_escape_string($_POST['korime']);
        $zaporka = $sql -> real_escape_string($_POST['zaporka']);
    ...
    function dohvatiKorisnika($user, $pass) {
        ...
        $sql = "SELECT korisnik.idkorisnik, korisnik.ime, korisnik.prezime, 
                ...
                FROM korisnik JOIN uloga ON korisnik.uloga_iduloga = uloga.iduloga WHERE kor_ime='$user'";
        $rs = $dbc->selectDB($sql);
        ...
        if ($broj == 1) {
            ...
            if (!strcmp($shaZaporka, $lozinka_kriptirana)) {
                if ($status == 'aktiviran') {
